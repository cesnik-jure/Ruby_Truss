...
img = Rasem::SVGImage.new($x_max+1, $y_max+1) do  
  ...
  @x_co_ori = $xy_node.column_vectors[0]
  @y_co_ori = $xy_node.column_vectors[1]
  @x_co_new = pt_g.xy_node.column_vectors[0]
  @y_co_new = pt_g.xy_node.column_vectors[1]
  ...
  text(30, 50, "Displacements of #{$truss_name} truss:", :font_family=>"Verdana",
  ...
  while n < @n_element do
    ...
    x_s = (@x_co_ori[s_node-1]+2)*@spacing
    y_s = @y_max - (2+@y_co_ori[s_node-1])*@spacing
    x_f = (@x_co_ori[f_node-1]+2)*@spacing
    y_f = @y_max - (2+@y_co_ori[f_node-1])*@spacing
  ...
  while n < @n_element do
    ...
    x_s = (@x_co_new[s_node-1]+2)*@spacing
    y_s = @y_max - (2+@y_co_new[s_node-1])*@spacing
    x_f = (@x_co_new[f_node-1]+2)*@spacing
    y_f = @y_max - (2+@y_co_new[f_node-1])*@spacing
  ...
  n = 0
  while n < @n_node do
    x = (@x_co_new[n]+2)*@spacing
    y = @y_max - (2+@y_co_new[n])*@spacing
